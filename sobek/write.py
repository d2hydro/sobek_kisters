...
_translate_link_class = {'SBK_PUMP': 'Pump',
                         'SBK_WEIR': 'Weir',
...
def kisters(sbk_case,name,link_classes,extra_params,prefix='',initials=False,rto_groups=None):
    ...
    lateral_links = sobek_network.objects.loc[(sobek_network.objects['TYPE'] == 'SBK_SBK-3B-REACH')]['LINK']
    lateral_nodes = sobek_network.links.loc[sobek_network.links['ID'].isin(lateral_links)]['TO_NODE'].values
    sobek_structures = sbk_case.network.objects.loc[sbk_case.network.objects['TYPE'].isin(['SBK_WEIR','SBK_PUMP','SBK_ORIFICE'])]
    sobek_cross_sections = sbk_case.network.objects.loc[sbk_case.network.objects['TYPE'] == 'SBK_PROFILE']
    ...
    def sbk_links(sobek_network):
        ...
        for index, row in sobek_network.links.to_crs(epsg='3857').iterrows():
            ...
            if link_id in sobek_structures['LINK'].values:
                structure = sobek_structures.loc[sobek_structures['LINK'] == link_id].iloc[0]
                ...
                if link_type == 'Pump':
                    ...
                    rto_params['max_flow'] = max([stage['capacity'] 
                                                  for 
                                                  stage 
                                                  in structure_params['pump_stages']])
                    rto_params['min_power'] = 0
                    rto_params['max_power'] = g * rho * rto_params['max_flow'] * structure_params['dh']
                ...
                if link_type == 'FlowControlledStructure':
                    ...
                    q = coeff * width * height * (2*g*(hus-(crest_level+coeff*height)))**(1/2)
                    rto_params['max_flow'] = q
             
            else:
                link_type = 'Channel'
                stations = []
                for idx, xs_row in sobek_cross_sections.loc[sobek_cross_sections['LINK'] == link_id].iterrows():
                    station = dict()
                    station["roughness"] = sbk_case.parameters.friction['global']['value']
                    station["distance"] = xs_row['LINK_POS']
                    station["cross_section"] = [{'z':z,'lr':y} for 
                                                y,z in 
                                                sbk_case.parameters.cross_sections[xs_row['ID']].items()]
                    if initials:
