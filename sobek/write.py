...
def kisters(
    ...
    """Write sobek case to kisters network."""
    authentication = OpenIDConnect(client_id=client_id,
    ...
    def sbk_nodes(sobek_network):
        ...
        for index, row in sobek_network.nodes.to_crs(epsg="3857").iterrows():
            if row["ID"] in sbk_case.boundaries.flow.index:
                node_type = _translate_boundary_class[
            ...
            if rto_groups:
                if row["group"]:
                    ...
                    rto_params = {key: value for key, value in rto_params.items()
                                  if not key == 'group_uid'}
        ...
        for index, row in sobek_network.links.to_crs(epsg="3857").iterrows():
            ...
            if link_id in sobek_structures["LINK"].values:
                ...
                rto_params['hydraulic_routing'] = {
                    "model": "saint-venant",
                    "roughness_model": sbk_case.parameters.friction["global"]["model"],
                    "stations": stations}
                if initials:
                    rto_params['hydraulic_routing'].update({
