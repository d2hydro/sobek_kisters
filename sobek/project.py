...
class Project(object):
    ...
    def __init__(self, path):
        with open(os.path.join(os.path.abspath(path), "caselist.cmt"), "r") as CSV:
            self.crs = "epsg:28992"
            self.path = Path(path)
            self.cache = Path(path, r"fixed\python_cache").absolute().resolve()
    ...
    def __getitem__(self, key):
        if key in list(self.__dict__.keys()):
            ...
            print(key, "not a case in this project")
    ...
    def drop_cache(self):
        """Drop all cache-files from the fixed/# folder."""
...
class Case(object):
    ...
    def to_kisters(
        ...
        """Write model to Kisters."""
        return write.kisters(
            self,
            name=name,
            client_id=client_id,
            ...
            hydraulic_routing_model=hydraulic_routing_model,
        )
